...
const PaymentCard = ({label, current, landing}: Props) => {
  return <div className={cn(
    ...
    <div className={cn(
        ...
        'flex flex-col rounded-xl pl-5 pr-10 bg-background-90'
    )}>
        {landing ? (
        ...
        </p>
        {
            ...
            {PLANS[label === 'PRO' ? 1 : 0].featuers.map((i) => (
            ...
            {landing ? (
                <Button className={cn(
                    'rounded-full mt-5',
                    label === 'PRO' ? 'bg-gradient-to-r from-indigo-500 text-white via-purple-500 to-pink-500' : 'bg-background-80 text-white hover:text-background-80'
                )}>{label === current ? 'Get Started' : current === 'PRO' ? 'Free' : 'Get Started'} </Button>
            ) : (
